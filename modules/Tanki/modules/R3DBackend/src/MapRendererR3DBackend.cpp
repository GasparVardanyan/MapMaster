			...
			for (const auto & [propName, propInfo] : props) {
				if (true == group.meshes.contains (propName)) {
					...
					for (const Map::MapObject & mapObject : propInfo) {
						...
						for (const PropMetaData::Mesh::Collider::RectCollider & rectCollider : collider.rectColliders) {
						...
						for (const PropMetaData::Mesh::Collider::BoxCollider & boxCollider : collider.boxColliders) {
...
void MapRendererR3DBackend::render (Camera & camera) {
	for (const SceneMesh & mesh : m_sceneObjects.meshes) {
		R3D_DrawMeshPro (* mesh.mesh, mesh.material, mesh.transform);
	}
...
void MapRendererR3DBackend::renderCollisionGeometry (bool wireframe) {
	for (const SceneMesh & mesh : m_sceneObjects.meshes) {
...
// cppcheck-suppress shadowFunction
void MapRendererR3DBackend::setMap (std::shared_ptr <Map> map) {
	m_map = std::move (map);
}

std::shared_ptr <Map> MapRendererR3DBackend::map () const {
	return m_map;
}

void MapRendererR3DBackend::setCollisionGeometryFaceColor (Color color) {
	m_collisionGeometryFaceColor = color;
}

void MapRendererR3DBackend::setCollisionGeometryEdgeColor (Color color) {
